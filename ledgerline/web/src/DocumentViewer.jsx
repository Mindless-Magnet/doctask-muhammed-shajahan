 ...
 * most of the time.
 */
export default function DocumentViewer({ pileId, documentsVersion, activeSpan, onClose }) {
  const [documents, setDocuments] = useState([]);
  const [openId, setOpenId] = useState(null);
  const [document_, setDocument] = useState(null);
  const [error, setError] = useState(null);

  // documentsVersion is bumped by the parent whenever the Documents tab adds something, so this
  // list refetches on the same signal instead of only on a pile switch — otherwise a document
  // uploaded moments ago would be missing here until something else happened to change pileId.
  useEffect(() => {
    if (!pileId) return;
