...
export default function Setup({ piles, pileId, onPileCreated, onChanged }) {
  ...
  async function startRun() {
    ...
    setError(null);
    try {
      // A document only just added has status "received" — nothing has classified or extracted it
      // yet. Naming only those ids is what makes a later run cost like an update instead of a full
      // re-run: everything already classified or extracted stays exactly as it was, proven rather
      // than reprocessed. Passing none (nothing new, or the very first run on this pile) falls back
      // to running the whole pile, which is correct the first time and only then.
      const newDocumentIds = documents
        .filter((doc) => doc.status === "received")
        .map((doc) => doc.document_id);
      const queued = await api.startRun(pileId, newDocumentIds);
      setStatus({ run_id: queued.run_id, status: queued.status });
      setNotice(
        newDocumentIds.length && newDocumentIds.length < documents.length
          ? `Started, scoped to the ${newDocumentIds.length} new document${newDocumentIds.length === 1 ? "" : "s"} — this costs like an update, not a full re-run.`
          : "Started. This takes a minute or two on a full file.",
      );
      ...
      polling.current = setInterval(async () => {
        try {
          ...
          if (["awaiting_approval", "committed", "failed"].includes(latest.run.status)) {
            ...
            setStarting(false);
            onChanged();
            // Every document this run touched just moved off status "received" in the database.
            // Without reloading here, this component's own list stays stale and the next run would
            // wrongly re-scope itself to documents that were already processed.
            loadDocuments();
            ...
            if (latest.run.status === "awaiting_approval") {
              setNotice("Finished reading. See what it found in the queue — nothing has been written yet.");
            }
          }
        } catch {
